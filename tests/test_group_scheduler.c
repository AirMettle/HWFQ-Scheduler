...
// Create a minimal parent scheduler for testing
static hwfq_scheduler_t *create_test_parent(void)
{
    hwfq_config_t config = {
        .max_tenants = 1000,
        .max_flows_per_tenant = 1000,
        .total_capacity = 1000000000ULL, // 1 GB/sec
        .alloc_fn = NULL,                // Use malloc
        .free_fn = NULL                  // Use free
    };
...
void test_single_entry_single_session(void)
{
    ...
    ret = group_scheduler_enqueue(gs, 0, 4096, (void *)0x1234, &session);
...
void test_multiple_sessions_same_entry(void)
{
    ...
    group_scheduler_enqueue(gs, 0, 4096, (void *)1, &s1);
    group_scheduler_enqueue(gs, 0, 8192, (void *)2, &s2);
    group_scheduler_enqueue(gs, 0, 2048, (void *)3, &s3);

    TEST_ASSERT(group_scheduler_get_session_count(gs) == 3, "Should have 3 sessions");

    // Dequeue and verify order (should be by finish time)
    ...
    // Verify finish times are in order
...
void test_multiple_entries_different_rates(void)
{
    ...
    // Enqueue same work size for each
    uint64_t work_size = 10000;
    group_scheduler_enqueue(gs, 0, work_size, (void *)0, NULL);
    group_scheduler_enqueue(gs, 1, work_size, (void *)1, NULL);
    group_scheduler_enqueue(gs, 2, work_size, (void *)2, NULL);

    // Higher rate should finish first (smaller service interval)
    ...
    TEST_ASSERT(session_get_entry_id(s1) == 2, "Entry 2 (highest rate) should finish first");
    TEST_ASSERT(session_get_entry_id(s2) == 1, "Entry 1 should finish second");
    TEST_ASSERT(session_get_entry_id(s3) == 0, "Entry 0 (lowest rate) should finish last");
...
void test_weight_based_allocation(void)
{
    ...
    group_entry_config_t entry0 = {
        .entry_id = 0, .allocation = {.allocation_type = HWFQ_ALLOCATION_WEIGHT, .weight = 70}};
    group_entry_config_t entry1 = {
        .entry_id = 1, .allocation = {.allocation_type = HWFQ_ALLOCATION_WEIGHT, .weight = 30}};
    ...
    group_scheduler_get_entry_rate(gs, 0, &rate0);
    group_scheduler_get_entry_rate(gs, 1, &rate1);

    // Rates should be proportional to weights
    // Entry 0 should get 70% of 1GB = 700MB
    // Entry 1 should get 30% of 1GB = 300MB
    TEST_ASSERT(rate0 > rate1, "Entry 0 should have higher rate");

    // Check ratio (allow some tolerance for integer division)
...
void test_mixed_rate_and_weight(void)
{
    ...
    // Entry 1: weight (should get proportional share of remaining 500MB)
    group_entry_config_t entry1 = {
        .entry_id = 1, .allocation = {.allocation_type = HWFQ_ALLOCATION_WEIGHT, .weight = 1}};
...
void test_virtual_time_advancement(void)
{
    ...
    // Enqueue and dequeue session
    group_scheduler_enqueue(gs, 0, 4096, NULL, NULL);
...
void test_entry_config_validation(void)
{
    ...
    group_entry_config_t invalid_rate = {
        .entry_id = 0, .allocation = {.allocation_type = HWFQ_ALLOCATION_RATE, .rate = 0}};
    ...
    group_entry_config_t invalid_weight = {
        .entry_id = 1, .allocation = {.allocation_type = HWFQ_ALLOCATION_WEIGHT, .weight = 0}};
...
void test_overbooking_prevention(void)
{
    ...
    // Try to add another entry that would exceed capacity
    group_entry_config_t entry1 = {
        .entry_id = 1,
        .allocation = {.allocation_type = HWFQ_ALLOCATION_RATE, .rate = 300000000ULL}};
...
void test_remove_entry(void)
{
    ...
    // Try to enqueue to removed entry
    ret = group_scheduler_enqueue(gs, 0, 4096, NULL, NULL);
